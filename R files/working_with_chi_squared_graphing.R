...
ggplot(gss1_hsjcsf, aes(x=satfin, y=Frequency, 
                  color=class_, group=class_)) +
  geom_line(size=1.5) +
...
tbjob <- table(gss1_hsjcsf$class_,gss1_hsjcsf$satjob)
tbjob
tb.prop <- prop.table(tbjob, 2)
tb.prop
chisq.test(tbjob)

tbjob.df <- as.data.frame(tb.prop)
tbjob.df
ggplot(tbjob.df, aes(x=Var2, y=Freq, color=Var1, group=Var1)) +
  geom_line()

ggplot(tbjob.df, aes(x=Var2, y=Freq, fill=Var1)) + geom_col()+
